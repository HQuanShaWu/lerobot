...
@PreTrainedConfig.register_subclass("rheovla")
@dataclass
class RheoVLAConfig(PreTrainedConfig):
    """Configuration for RheoVLA policy wrapper (copied from Groot)."""
    ...
    # Image preprocessing (adjust to match RheoVLA's expected input)
    image_size: tuple[int, int] = (224, 224)

    # RheoVLA-specific model parameters (from groot_finetune_script.py)

    # Path or HuggingFace model ID for the base RheoVLA model
    ...
    def validate_features(self) -> None:
        """Validate and set up input/output features for RheoVLA."""
        image_features = [key for key, feat in self.input_features.items() if feat.type == FeatureType.VISUAL]
        if not image_features:
            raise ValueError(
                "RheoVLA policy requires at least one visual input feature. "
    ...
    def observation_delta_indices(self) -> None:
        """Return indices for delta observations (None for RheoVLA)."""
        return None
    ...
    def reward_delta_indices(self) -> None:
        """Return indices for delta rewards (None for RheoVLA)."""
        return None
